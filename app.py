...
TEAM_LOGO_ALIASES = {
    ...
    "PDX": "POR", "POR": "POR", "PORTLAND": "POR", "PORTLAND FIRE": "POR",
...
def select_primary_prop_lines(df: pd.DataFrame, master: pd.DataFrame = None, group_source: bool = False) -> pd.DataFrame:
    ...
    group_cols_for_count = ["NameKey", "Market"] + (["Source"] if group_source and "Source" in out.columns else [])
    out["_variant_count"] = out.groupby(group_cols_for_count)["Line"].transform("nunique")
    out["_ud_base_score"] = pd.to_numeric(out.get("UD Base Score", pd.Series(0, index=out.index)), errors="coerce").fillna(0)
    out["_ud_order"] = pd.to_numeric(out.get("UD Candidate Order", pd.Series(999999, index=out.index)), errors="coerce").fillna(999999)
    ...
    out = out.sort_values(
        sort_cols + ["_ud_base_score", "_ud_order", "_mainline_score", "Line"],
        ascending=[True] * len(sort_cols) + [False, True, True, True],
    )
    keep_cols = sort_cols
    out = out.drop_duplicates(subset=keep_cols, keep="first")
    out["Line Selection"] = np.where(
        pd.to_numeric(out.get("_ud_base_score"), errors="coerce").fillna(0).gt(0),
        "UNDERDOG_BASE_CARD_ORDER",
        np.where(pd.to_numeric(out.get("_variant_count"), errors="coerce").fillna(1).gt(1),
        "UNDERDOG_FIRST_DUPLICATE_FALLBACK",
        np.where(pd.to_numeric(out.get("_mainline_expected"), errors="coerce").notna(),
        "PLAYER_BASELINE_MAINLINE",
        "MARKET_RANGE_MAINLINE")),
    )
    return out.drop(columns=["_mainline_expected", "_mainline_score", "_variant_count", "_ud_base_score", "_ud_order"], errors="ignore")
...
def fetch_underdog_board():
    ...
    if not master.empty and "Player" in master.columns:
        for _, r in master.dropna(subset=["Player"]).iterrows():
            ...
            toks = nk.split()
            team_raw = str(r.get("Team", "")).upper().strip()
            try:
                team_key = _team_key_for_matchup(team_raw) if "_team_key_for_matchup" in globals() else team_abbrev(team_raw)
            except Exception:
                team_key = team_raw
            player_pool.append({
                "Player": player,
    ...
    def active_schedule_teams():
        ...
        for c in ["Home", "Away"]:
            if c in s.columns:
                for x in s[c].dropna().tolist():
                    key = _team_key_for_matchup(x) if "_team_key_for_matchup" in globals() else team_abbrev(x)
                    if key:
                        teams.add(key)
    ...
    def _ud_line_profile(o, raw=""):
        a = attr(o)
        raw_txt = (json.dumps(a, default=str) + " " + str(raw or "")).lower()
        alt_words = [
            "alternate", "alt ", "ladder", "discount", "discounted", "boost",
            "special", "rival", "combo", "streak", "insurance", "promo",
        ]
        bad_alt = any(w in raw_txt for w in alt_words)
        two_sided = _ud_has_two_sided_options(o)
        status_txt = " ".join(str(a.get(k, "")) for k in ["status", "state", "display_status"]).lower()
        active = not any(x in status_txt for x in ["suspended", "closed", "removed", "settled", "graded"])
        has_line_key = any(a.get(k) not in [None, ""] for k in ["stat_value", "line_score", "target_value"])
        score = 0
        if two_sided:
            score += 5
        if active:
        ...
        if bad_alt:
            score -= 8
        kind = "base-card" if score > 0 and not bad_alt else "alternate-or-unknown"
        return {
            "UD Base Score": int(score),
            "UD Line Kind": kind,
            "UD Two Sided": bool(two_sided),
        }
    ...
    def append_row(player, team, market, line, raw, parser, order=999999, profile=None, game_ctx=None):
        ...
        rows.append({
            ...
            "UD Candidate Order": int(order),
            "UD Base Score": profile.get("UD Base Score", 0),
            "UD Line Kind": profile.get("UD Line Kind", ""),
            "UD Two Sided": profile.get("UD Two Sided", False),
        })
        return True
    ...
    for url in UNDERDOG_URLS:
        ...
        for order, lo in enumerate(line_objs):
            ...
            if pd.isna(line):
                ...
                continue
            profile = _ud_line_profile(lo, raw)
    ...
    if rows:
        ...
        debug.append({"source":"Underdog", "url":"parser", "status":"ok", "rows":len(df), "message":f"accepted {len(df)} Underdog base-card line rows from {before_n} candidates; decode rows {len(decode_df)}"})
...
def team_abbrev(x) -> str:
    ...
    if "_team_key_for_matchup" in globals():
        try:
            key = _team_key_for_matchup(s)
            if key:
                return key
        except Exception:
            pass
...
def result_record_summary(df: pd.DataFrame) -> Dict[str, Any]:
    if df is None or df.empty or "Result" not in df.columns:
        return {"total": 0, "wins": 0, "losses": 0, "pushes": 0, "voids": 0, "decisions": 0, "win_rate": np.nan}
    result = df["Result"].astype(str).str.upper()
...
def attach_result_symbols(df: pd.DataFrame) -> pd.DataFrame:
    if df is None or df.empty:
        return pd.DataFrame()
    out = df.copy()
    if "Result" not in out.columns:
        out["Result"] = ""
    result = out["Result"].astype(str).str.upper()
    out["✅/❌"] = np.select(
        [result.eq("WIN"), result.eq("LOSS"), result.eq("PUSH"), result.eq("VOID"), result.eq("PENDING")],
        ["✅", "❌", "➖", "🚫", "⏳"],
        default="",
    )
    cols = ["✅/❌"] + [c for c in out.columns if c != "✅/❌"]
    return out[cols]
...
def hero_panel(board_rows: int = 0, real_lines: int = 0, no_line: int = 0, strong: int = 0):
    ...
    """, unsafe_allow_html=True)
    c0, c1, c2 = st.columns([0.8, 1.2, 1.4])
    with c0:
...
def _slate_matchup_set(mode: str) -> set:
    """Return canonical away @ home matchups for the selected slate."""
    try:
        sched = schedule_for_slate(mode)
        target = slate_target_date(mode)
        if (sched is None or sched.empty) and target is not None and "fetch_espn_wnba_schedule_for_date" in globals():
        ...
        if sched is None or sched.empty:
            return set()
        away_col = next((c for c in ["Away", "AwayTeam", "Visitor", "VisitorTeam"] if c in sched.columns), None)
        home_col = next((c for c in ["Home", "HomeTeam"] if c in sched.columns), None)
        if not away_col or not home_col:
            return set()
        out = set()
        for _, rr in sched.iterrows():
            away = team_abbrev(rr.get(away_col))
            home = team_abbrev(rr.get(home_col))
            if away and home:
                out.add(f"{away} @ {home}".upper())
                out.add(f"{away} VS {home}".upper())
                out.add(f"{home} VS {away}".upper())
        return out
    except Exception:
        return set()


def filter_projection_board_for_slate(board: pd.DataFrame, mode: str) -> pd.DataFrame:
    """Final display guard so cached boards cannot leak into the wrong slate."""
    if board is None or board.empty or mode == "All Lines":
        return board.copy() if board is not None else pd.DataFrame()
    target = slate_target_date(mode)
    d = board.copy()
    keep = pd.Series(False, index=d.index)
    if target is not None and "SlateDate" in d.columns:
        keep = keep | d["SlateDate"].astype(str).eq(str(target))
    if "Slate" in d.columns:
        keep = keep | d["Slate"].astype(str).str.lower().eq(str(mode).lower())
    slate_matchups = _slate_matchup_set(mode)
    if slate_matchups and "Matchup" in d.columns:
        matchup_text = d["Matchup"].astype(str).str.upper().str.replace(" VS ", " @ ", regex=False)
        keep = keep | matchup_text.isin(slate_matchups)
    return d[keep].copy()


def _ensure_line_matchups_for_filter(lines: pd.DataFrame) -> pd.DataFrame:
    """Best-effort matchup fill before Today/Tomorrow filtering.

    Underdog rows can arrive with Team/Raw populated but no Matchup/Opponent yet.
    If we filter before extracting the raw event, a partial schedule can drop
    valid games. This fills matchup text only; it does not change line values.
    """
    if lines is None or lines.empty:
        return pd.DataFrame()
    d = lines.copy()
    for c in ["Team", "Opponent", "Matchup", "HomeAway"]:
        if c not in d.columns:
            d[c] = ""
    if "_parse_event_teams_from_text" not in globals():
        return d
    for idx, r in d.iterrows():
        current_matchup = str(r.get("Matchup", "") or "").strip()
        current_opp = str(r.get("Opponent", "") or "").strip()
        if current_matchup and current_opp:
            continue
        raw_candidates = [r.get("Raw"), r.get("Event"), r.get("Game"), r.get("GameInfo"), r.get("Description")]
        away = home = ""
        for raw in raw_candidates:
            away, home = _parse_event_teams_from_text(raw)
            if away and home:
                break
        if not away or not home:
            continue
        team = _team_key_for_matchup(r.get("Team")) if "_team_key_for_matchup" in globals() else team_abbrev(r.get("Team"))
        d.at[idx, "Matchup"] = f"{away} @ {home}"
        if team == away:
            d.at[idx, "Opponent"] = home
            d.at[idx, "HomeAway"] = "AWAY"
        elif team == home:
            d.at[idx, "Opponent"] = away
            d.at[idx, "HomeAway"] = "HOME"
    return d
...
def filter_lines_for_slate(lines: pd.DataFrame, mode: str) -> Tuple[pd.DataFrame, str]:
    ...
    d = _ensure_line_matchups_for_filter(lines)
    slate_teams = _slate_team_set(mode)
    slate_matchups = _slate_matchup_set(mode) if "_slate_matchup_set" in globals() else set()
    ...
    # 3) Underdog WNBA often lacks start time. Prefer verified matchup pairs,
    # then fall back to teams only when the row has no matchup text.
    if slate_matchups or slate_teams:
        matchup_match = pd.Series(False, index=d.index)
        if slate_matchups and "Matchup" in d.columns:
            matchup_text = d["Matchup"].astype(str).str.upper().str.replace(" VS ", " @ ", regex=False)
            matchup_match = matchup_text.isin(slate_matchups)
        team_match = pd.Series(False, index=d.index)
        for c in ["Team", "Opponent"]:
            if c in d.columns:
                team_match = team_match | d[c].astype(str).map(team_abbrev).isin(slate_teams)
        row_has_matchup = d.get("Matchup", pd.Series("", index=d.index)).astype(str).str.strip().ne("")
        no_matchup_team_fallback = (~row_has_matchup) & team_match
        # Only apply schedule fallback to rows without dates; dated rows must match target.
        keep = keep | ((~has_dates) & (matchup_match | no_matchup_team_fallback))
    ...
    # If the schedule feed is incomplete but Underdog provided no start times,
    # keep the pulled active slate instead of returning 0 cards. Dated rows
    # still must match the selected date.
    if not has_dates.any() and not d.empty:
        note = f"Schedule did not verify {mode.lower()} props, so using active pulled line matchups from Underdog/manual rows."
        return d.copy(), note
...
def pull_board_lines(use_ud_flag: bool, use_sleeper_flag: bool = False, use_odds_api_flag: bool = False, odds_api_key: str = "") -> Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    try:
        if lines is not None and not lines.empty:
            persisted = lines.copy()
            persisted["LineParserVersion"] = LINE_PARSER_VERSION
            persisted["SavedBoardAt"] = now_iso()
            persisted.to_csv(SAVED_LINES_FILE, index=False)
    except Exception as _persist_exc:
        st.session_state["wnba_saved_lines_error"] = str(_persist_exc)[:180]
    try:
...
def _market_why_df(r: pd.Series) -> pd.DataFrame:
    rows = []
    for label, col in [
        ("Projection reason", "Projection Explanation"),
        ("Recent / hit-rate support", "Hit Rate Context"),
        ("Recent support", "Recent Support"),
        ("Veteran / capability", "Veteran Capability"),
        ("Game script", "WNBA ML Game Script"),
        ("Game script note", "WNBA ML Game Script Note"),
        ("Risk / no-bet flags", "No-Bet Risk Flags"),
        ("Pass / gate note", "PASS Reason"),
        ("Winning gate", "Winning Gate Note"),
        ("Line selection", "Line Selection"),
        ("UD line kind", "UD Line Kind"),
        ("Projection integrity", "Projection Integrity Note"),
    ]:
        val = str(r.get(col, "") or "").strip()
        if val and val.lower() not in {"nan", "none"}:
            rows.append({"Field": label, "Value": re.sub(r"\s+", " ", re.sub(r"<[^>]+>", " ", val)).strip()})
    if not rows:
        rows.append({"Field": "Details", "Value": "No extra notes available for this market."})
    return pd.DataFrame(rows)
...
def render_grouped_player_card(player_df: pd.DataFrame):
    ...
    """, unsafe_allow_html=True)
    for _, rr in player_df.iterrows():
        m = str(rr.get("Market", "PROP")).upper()
        lean = str(rr.get("Lean", "TRACK")).upper()
...
def render_grouped_table_or_cards(proj_df: pd.DataFrame, mode: str, key_prefix: str, default_cards: bool = False) -> pd.DataFrame:
    """Authoritative Player Cards renderer: always use the compact pro-row board.

    This deliberately ignores any legacy Streamlit radio/session-state value so a
    prior selection of the old grouped-card view cannot make the production Player
    Cards tab switch layouts after a deploy or rerun.
    """
    if proj_df is None or proj_df.empty:
    ...
    with ac1:
        if st.button(f"✅ Save {mode} Official Before", key=f"{key_prefix}_save_before", use_container_width=True):
            ...
            snapshot_n = save_board_snapshot(view_df, st.session_state.get("wnba_lines_all", pd.DataFrame()), mode)
            st.session_state["wnba_prefer_projection_cache_after_refresh"] = False
            st.success(f"Saved {n} tracked plays and {snapshot_n} board rows. {mode} will reload without refreshing.")
    with ac2:
    ...
    # IMPORTANT: This is the exact compact six-column renderer shown in the
    # user's reference screenshot. No legacy grouped-card/session-state branch.
...
def _moneyline_pairs_from_schedule(mode: str = "Today") -> List[Dict[str, Any]]:
    """Resolve Moneyline games from persisted schedule sources.

    Exact slate date remains first choice. If a provider stores the same evening
    game up to two calendar days away, allow a +/-2 day recovery only when the teams
    overlap the currently pulled/saved WNBA prop slate. This fixes timezone/date
    drift without mixing unrelated games.
    """
    target = slate_target_date(mode)
    schedule_frames = []

    # Teams from current or persisted lines are the safety key for soft-date recovery.
    active_teams = set()
    try:
        active_lines = st.session_state.get("wnba_lines_all", pd.DataFrame())
        if active_lines is None or active_lines.empty:
            active_lines = load_saved_lines_snapshot()
        if active_lines is not None and not active_lines.empty:
            ld = _ensure_line_matchups_for_filter(active_lines)
            for c in ["Team", "Opponent"]:
                if c in ld.columns:
                    active_teams.update({_team_key_for_matchup(x) for x in ld[c].dropna().tolist() if _team_key_for_matchup(x)})
            if "Matchup" in ld.columns:
                for raw in ld["Matchup"].dropna().astype(str).tolist():
                    a,b = _parse_event_teams_from_text(raw)
                    if a: active_teams.add(a)
                    if b: active_teams.add(b)
    except Exception:
        active_teams = set()

    try:
        ud_all = st.session_state.get("wnba_moneyline_games", pd.DataFrame())
        if (ud_all is None or ud_all.empty) and UNDERDOG_GAMES_CACHE_FILE.exists():
            ud_all = pd.read_csv(UNDERDOG_GAMES_CACHE_FILE, low_memory=False)
        if ud_all is not None and not ud_all.empty:
            ud = ud_all.copy()
            exact = pd.DataFrame()
            if target is not None:
                if "SlateDate" in ud.columns:
                    exact = ud[ud["SlateDate"].astype(str).eq(str(target))].copy()
                if exact.empty and "GameDate" in ud.columns:
                    gd = pd.to_datetime(ud["GameDate"], errors="coerce", utc=True)
                    exact = ud[gd.dt.date == target].copy()
            else:
                exact = ud.copy()
            if not exact.empty:
                schedule_frames.append(exact)
            elif target is not None:
                # Soft recovery: +/-2 days, but only teams present in today's pulled slate.
                dates = pd.to_datetime(ud.get("SlateDate", ud.get("GameDate")), errors="coerce", utc=True)
                if dates.notna().any():
                    daydiff = dates.dt.date.map(lambda x: abs((x-target).days) if x is not None else 99)
                    near = ud[daydiff <= 2].copy()
                    if active_teams and not near.empty:
                        keep=[]
                        for _,r in near.iterrows():
                            a=_team_key_for_matchup(r.get("Away")); b=_team_key_for_matchup(r.get("Home"))
                            keep.append(bool(a in active_teams or b in active_teams))
                        near=near[pd.Series(keep,index=near.index)]
                    if not near.empty:
                        schedule_frames.append(near)
                        st.session_state["wnba_moneyline_schedule_soft_date"] = True
    except Exception as exc:
    ...
    # Last schedule fallback: nearby cached schedule games that overlap active prop teams.
    if not schedule_frames and target is not None and active_teams:
        try:
            full = standardize_schedules(load_dataset("schedules"))
            if full is not None and not full.empty and "GameDate" in full.columns:
                gd = pd.to_datetime(full["GameDate"], errors="coerce")
                diff = gd.dt.date.map(lambda x: abs((x-target).days) if x is not None else 99)
                near = full[diff <= 2].copy()
                if not near.empty:
                    near = near[
                        near.get("Away", "").map(_team_key_for_matchup).isin(active_teams)
                        | near.get("Home", "").map(_team_key_for_matchup).isin(active_teams)
                    ].copy()
                if not near.empty:
                    schedule_frames.append(near)
        except Exception as exc:
    ...
    for _,r in sched.iterrows():
        away=_team_key_for_matchup(r.get("Away")); home=_team_key_for_matchup(r.get("Home"))
        if not away or not home or away==home: continue
        key=tuple(sorted([away,home]))
        if key in seen: continue
        seen.add(key)
        pairs.append({
            "key":key,"team":away,"opp":home,"matchup":f"{away} @ {home}",
            "source":str(r.get("Source","schedule") or "schedule"),
            "game_date":str(r.get("GameDate",r.get("SlateDate","")))[:19],
            "game_id":str(r.get("GameID","") or ""),
        })
    return pairs


def _moneyline_pairs_from_lines(lines_df: pd.DataFrame) -> List[Dict[str, Any]]:
    if lines_df is None or lines_df.empty:
        try:
            lines_df = load_saved_lines_snapshot()
        except Exception:
            lines_df = pd.DataFrame()
    if lines_df is None or lines_df.empty:
        return []
    d = _ensure_line_matchups_for_filter(lines_df)
    pairs = []
    seen = set()
    if "Matchup" in d.columns:
        for matchup in d["Matchup"].dropna().astype(str).unique().tolist():
            a, b = _parse_event_teams_from_text(matchup)
            if not a or not b:
                continue
            ...
            if key in seen:
                continue
            seen.add(key)
            pairs.append({"key": key, "team": a, "opp": b, "matchup": f"{a} @ {b}", "source": "pulled lines", "game_date": ""})
    if {"Team", "Opponent"}.issubset(d.columns):
        for _, r in d[["Team", "Opponent"]].drop_duplicates().iterrows():
            ...
            seen.add(key)
            pairs.append({"key": key, "team": a, "opp": b, "matchup": f"{a} @ {b}", "source": "pulled lines", "game_date": ""})
    for raw_col in ["Raw", "Event", "Game", "GameInfo", "Description"]:
        if raw_col not in d.columns or "_parse_event_teams_from_text" not in globals():
            continue
        for raw in d[raw_col].dropna().astype(str).unique().tolist():
            a, b = _parse_event_teams_from_text(raw)
            if not a or not b or a == b:
                continue
            key = tuple(sorted([a, b]))
            if key in seen:
                continue
            seen.add(key)
            pairs.append({"key": key, "team": a, "opp": b, "matchup": f"{a} @ {b}", "source": f"pulled {raw_col}", "game_date": ""})
    return pairs
...
def _moneyline_pairs_from_board(board_df: pd.DataFrame) -> List[Dict[str, Any]]:
    if board_df is None or board_df.empty:
        try:
            board_df = load_dataset("projection_board")
        except Exception:
...
def render_wnba_ml_system(board_df: pd.DataFrame, key_prefix: str = "ml_system", mode: str = "Today") -> pd.DataFrame:
    ...
    if ml_lines is None or ml_lines.empty:
        try:
            ml_lines = load_saved_lines_snapshot()
        except Exception:
    ...
    if schedule_pairs:
        ...
        st.warning("Moneyline is using pulled-line/prop-board matchups because today's live schedule could not be loaded.")
    with st.expander("Moneyline coverage debug", expanded=False):
        st.write({
            "schedule_pairs": [p.get("matchup") for p in schedule_pairs],
    ...
    ml_html += ".wnba-ml-total{margin-top:18px;background:rgba(16,23,34,.96);border:1px solid rgba(148,163,184,.10);border-radius:12px;padding:14px;position:relative}.wnba-ml-total small{display:block;color:#7c8798;text-transform:uppercase;font-size:.68rem}.wnba-ml-total strong{font-size:2.7rem;color:#fb3f7c;font-weight:400}.wnba-ml-total span{margin-left:10px;color:#4ade80;font-weight:900}.wnba-ml-total span.under{color:#60a5fa}.wnba-ml-total-side{position:absolute;right:18px;top:22px;color:#fbbf24;font-size:1.25rem}.wnba-ml-total em{position:absolute;right:18px;top:52px;color:#fef3c7;border:1px solid rgba(250,204,21,.55);border-radius:999px;padding:2px 8px;font-style:normal;font-size:.72rem}.wnba-ml-total-bar{height:8px;background:#1f2937;border-radius:999px;overflow:hidden;margin-top:10px}.wnba-ml-total-bar i{display:block;height:100%;background:linear-gradient(90deg,#f59e0b,#facc15)}.wnba-ml-total p{display:flex;justify-content:space-between;color:#fbbf24;font-size:.72rem;margin:6px 0 0}.wnba-ml-total p span{margin:0;color:#94a3b8}"
    ml_html += ".wnba-ml-bottom{display:grid;grid-template-columns:repeat(5,1fr);gap:8px;margin-top:10px}.wnba-ml-bottom div{background:rgba(16,23,34,.96);border:1px solid rgba(148,163,184,.08);border-radius:9px;padding:9px;text-align:center;min-width:0}.wnba-ml-bottom small{display:block;color:#7c8798;text-transform:uppercase;font-size:.62rem}.wnba-ml-bottom b{display:block;color:#fbbf24}.wnba-ml-bottom span,.wnba-ml-bottom em{display:block;color:#94a3b8;font-size:.72rem;font-style:normal}.wnba-ml-sim{margin-top:10px;color:#c4b5fd;font-size:.78rem;line-height:1.35;white-space:normal}"
...
def _render_grouped_projection_df(proj_df: pd.DataFrame, mode: str, search_key: str, max_key: str, official_key: str, saved_view: bool = False) -> pd.DataFrame:
    ...
    proj_df = filter_projection_board_for_slate(proj_df, mode)
    if proj_df is None or proj_df.empty:
...
def _load_last_good_projection_board(mode: str = "All Lines") -> tuple[pd.DataFrame, str]:
    """Load the same last-good projection rows used by Best Bets.

    This is display-only fallback logic. It never changes, re-selects, or
    recalculates an Underdog line. Live refresh remains the source of new rows.
    """
    candidates = []
    try:
        candidates.append((CACHE_FILES.get("projection_board"), "projection cache"))
    except Exception:
        pass
    try:
        candidates.append((SAVED_BOARD_FILE, "saved board"))
    except Exception:
        pass
    ...
    for path_obj, label in candidates:
        try:
            ...
            # Only real projection rows belong on Player Cards.
            if "Player" not in df.columns or "Market" not in df.columns or "Line" not in df.columns:
                continue
            df = df.copy()
            df["Line"] = pd.to_numeric(df["Line"], errors="coerce")
            ...
            mode_txt = str(mode or "All Lines")
            if mode_txt != "All Lines":
                filtered = pd.DataFrame()
                if "Slate" in df.columns:
                    filtered = df[df["Slate"].astype(str).str.lower().eq(mode_txt.lower())].copy()
                if filtered.empty and "SlateDate" in df.columns:
                    try:
                        target = slate_target_date(mode_txt)
                    except Exception:
                        target = None
                    if target is not None:
                        filtered = df[df["SlateDate"].astype(str).eq(str(target))].copy()
                # Never leak a different-day board into Today/Tomorrow.
                if filtered.empty:
                    continue
                df = filtered

            # Preserve the exact line rows but remove accidental duplicate renders.
            dedupe = [c for c in ["Player", "Market", "Line", "Team", "Opponent"] if c in df.columns]
            if dedupe:
...
def render_grouped_player_board(mode: str, use_ud_flag: bool, logs_global: pd.DataFrame, master_global: pd.DataFrame):
    st.markdown(f"<div class='section-title'>{mode} — Player Cards</div>", unsafe_allow_html=True)
    ...
    with top_cols[2]:
        _cache_preview, _cache_preview_label = _load_last_good_projection_board(mode)
        ...
        if _db_n > 0:
            st.metric("Database players", _db_n)
        else:
            st.metric("Board players", _board_n)
    with top_cols[3]:
        st.caption(f"Compact rows: Player / Prop / Proj / Edge / Pick / Over %. Full-game line guard: {LINE_PARSER_VERSION}. Old saved boards from the previous parser are automatically ignored.")
    ...
    # the same last-good projection cache immediately. The Refresh Live button
    # above is the only action that forces a new Underdog request.
...
# ============================================================================
# v3.4.4 — CALIBRATED TRUE-SIDE ENGINE
# Goals:
#   • no duplicate pace/matchup/role boosts
#   • robust player baseline with conservative recent-form weighting
#   • market-aware Bayesian shrinkage (small, confidence-dependent)
#   • PRA equals corrected PTS + REB + AST exactly
#   • calibrated probabilities with uncertainty inflation
...
def _v344_player_lookup(base: pd.DataFrame) -> pd.DataFrame:
    if base is None or base.empty:
        return pd.DataFrame()
    b = base.copy()
...
def _v344_value(br, row, names, fallback=np.nan):
    for name in names:
        try:
            if br is not None and name in br.index:
            ...
            pass
        try:
            v = safe_float(row.get(name), np.nan)
            if pd.notna(v):
                return v
        except Exception:
            pass
...
def _v344_baseline(row: pd.Series, br: Optional[pd.Series], market: str):
    vals = {
        "l5": _v344_value(br, row, [f"{market}_l5", "L5 Avg"]),
        "l10": _v344_value(br, row, [f"{market}_l10", "L10 Avg"]),
        "l20": _v344_value(br, row, [f"{market}_l20", "L20 Avg"]),
        "season": _v344_value(br, row, [f"{market}_avg", "Season Avg"]),
        "prior": _v344_value(br, row, [f"{market}_prior"]),
    }
    available = _v344_numeric(vals.values())
    if not available:
        return safe_float(row.get("Projection"), np.nan), vals
    med = float(np.median(available))
    for k in vals:
        if pd.isna(vals[k]):
            vals[k] = med
    # L5 is intentionally capped in influence. L10/L20/season establish the
    # sustainable level; prior protects small samples and rookies.
    anchor = (
        0.16 * vals["l5"]
        + 0.26 * vals["l10"]
        + 0.22 * vals["l20"]
    ...
    # Winsorize the anchor around the robust center of all windows.
...
def _v344_minutes_multiplier(row: pd.Series, br: Optional[pd.Series]) -> tuple:
    projected = safe_float(row.get("MIN Proj"), np.nan)
    l5 = _v344_value(br, row, ["MIN_l5"])
    l10 = _v344_value(br, row, ["MIN_l10"])
    season = _v344_value(br, row, ["MIN_avg", "MIN"])
    recent = _v344_numeric([l5, l10, season])
    if pd.isna(projected) or not recent:
        return 1.0, 55.0
    baseline = 0.35*(l5 if pd.notna(l5) else np.median(recent)) + 0.40*(l10 if pd.notna(l10) else np.median(recent)) + 0.25*(season if pd.notna(season) else np.median(recent))
    if baseline <= 0:
        return 1.0, 55.0
    raw = projected / baseline
    dispersion = float(np.std(recent)) if len(recent) >= 2 else 2.5
    confidence = float(np.clip(96.0 - dispersion*8.0 - abs(projected-baseline)*2.2, 45.0, 96.0))
    # Only a verified role change may move the projection by more than 4%.
    role_text = " ".join(str(row.get(c, "")) for c in ["Injury Status", "Role", "Starter Status", "Lineup Status", "Availability"] ).upper()
    verified_change = any(t in role_text for t in ["CONFIRMED START", "STARTER", "OUT", "INACTIVE", "ROLE CHANGE"])
    cap = 0.06 if verified_change and confidence >= 75 else 0.04
    return float(np.clip(raw, 1-cap, 1+cap)), confidence


def _v344_matchup_multiplier(row: pd.Series) -> tuple:
    integrity = str(row.get("Data Integrity", "LIMITED")).upper()
    opp = str(row.get("Opponent", "")).strip()
    drtg = safe_float(row.get("Opponent DRtg"), np.nan)
    ...
    verified = integrity == "VERIFIED" and opp and opp.lower() not in {"nan", "none", "opponent unavailable"} and pd.notna(drtg) and pd.notna(pace) and pd.notna(rank)
    if not verified:
        return 1.0, "Unknown", 35.0
    # The upstream matchup score already combines pace/DRtg/position context.
    # Apply one small bounded move only. Rank is used as confirmation, not a
    # second boost.
    score_move = float(np.clip((score - 50.0) / 50.0 * 0.0125, -0.0125, 0.0125))
    rank_move = 0.0
    if pd.notna(teams) and teams >= 6:
        pct = (rank - 1.0) / max(1.0, teams - 1.0)
        rank_move = float(np.clip((pct - 0.5) * 0.005, -0.0025, 0.0025))
    mult = float(np.clip(1.0 + score_move + rank_move, 0.985, 1.015))
    grade = "Favorable" if score >= 58 else "Tough" if score <= 42 else "Neutral"
    confidence = float(np.clip(70 + min(20, teams if pd.notna(teams) else 0), 70, 92))
    return mult, grade, confidence


def _v344_market_shrink(proj: float, line: float, integrity: str, sample_quality: float) -> tuple:
    if pd.isna(proj) or pd.isna(line):
        return proj, 0.0
    # Sportsbook consensus is informative but never replaces the model.
    # Verified/high-sample projections receive less market shrinkage.
    base_w = 0.12 if integrity == "VERIFIED" else 0.22
    w = float(np.clip(base_w + (1.0-sample_quality)*0.10, 0.10, 0.30))
    return float((1.0-w)*proj + w*line), w


def _v344_component_projection(row: pd.Series, br: Optional[pd.Series], market: str):
    anchor, vals = _v344_baseline(row, br, market)
    if pd.isna(anchor):
        return np.nan, vals, {}
    min_mult, min_conf = _v344_minutes_multiplier(row, br)
    matchup_mult, matchup_grade, matchup_conf = _v344_matchup_multiplier(row)
    raw = anchor * min_mult * matchup_mult
    # Robust sanity cap around L10/L20/season; this is not a market cap.
    stable = _v344_numeric([vals.get("l10"), vals.get("l20"), vals.get("season"), vals.get("prior")])
    if stable:
        center = float(np.median(stable))
        sd_proxy = float(np.std(stable)) if len(stable) >= 2 else 0.0
        width = max({"PTS":3.2,"REB":1.45,"AST":1.35,"PRA":4.8}.get(market,2.5), 2.25*sd_proxy)
        raw = float(np.clip(raw, max(0.0, center-width), center+width))
    line = safe_float(row.get("Line"), np.nan)
    sample_n = safe_float(_v344_value(br, row, [f"{market}_Games", "Games", "GP"]), 15.0)
    sample_quality = float(np.clip(sample_n/25.0, 0.35, 1.0))
    integrity = str(row.get("Data Integrity", "LIMITED")).upper()
    final, market_w = _v344_market_shrink(raw, line, integrity, sample_quality)
    meta = {
        "anchor": anchor, "minutes_mult": min_mult, "minutes_conf": min_conf,
        "matchup_mult": matchup_mult, "matchup_grade": matchup_grade,
        "matchup_conf": matchup_conf, "market_weight": market_w,
        "sample_quality": sample_quality,
    }
    return final, vals, meta


def _v344_sd(br: Optional[pd.Series], market: str, projection: float, data_conf: float) -> float:
    default = {"PTS":4.9,"REB":2.45,"AST":2.25,"PRA":6.8}.get(market,4.0)
    sd = _v344_value(br, pd.Series(dtype=object), [f"{market}_Std20", f"{market}_std"], default)
    floor = {"PTS":3.6,"REB":1.65,"AST":1.55,"PRA":5.1}.get(market,2.2)
    ceiling = {"PTS":7.8,"REB":4.5,"AST":4.2,"PRA":11.5}.get(market,8.0)
    # Missing/limited data increases uncertainty rather than shifting the mean.
    uncertainty = 1.0 + max(0.0, 82.0-data_conf)/100.0
    return float(np.clip(sd*uncertainty, floor, ceiling))


def _v344_probability(line: float, mean: float, sd: float) -> float:
    if pd.isna(line) or pd.isna(mean) or pd.isna(sd) or sd <= 0:
        return 50.0
    # Half-point continuity correction prevents tiny edges from looking too strong.
    return float(np.clip(_normal_over_probability(line + 0.15, mean, sd), 1.0, 99.0))


def _v344_rebuild_board(board: pd.DataFrame, base: pd.DataFrame) -> pd.DataFrame:
    if board is None or board.empty:
        return board
    lookup = _v344_player_lookup(base)
    rows = []
    component_cache = {}
    for _, rr in board.iterrows():
        row = rr.copy()
        ...
        market = str(row.get("Market", "PTS")).upper()
        cache_key = (key, str(row.get("Team", "")), str(row.get("Opponent", "")), safe_float(row.get("MIN Proj"), np.nan))
        if cache_key not in component_cache:
            comps, values, metas = {}, {}, {}
            for m in ["PTS","REB","AST"]:
                p, v, meta = _v344_component_projection(row, br, m)
                comps[m], values[m], metas[m] = p, v, meta
            component_cache[cache_key] = (comps, values, metas)
        comps, values, metas = component_cache[cache_key]

        if market == "PRA" and all(pd.notna(comps.get(m)) for m in ["PTS","REB","AST"]):
            # Exact component identity: no second PRA matchup, pace, minutes or cap.
            final_proj = float(comps["PTS"] + comps["REB"] + comps["AST"])
            vals = {k: sum(safe_float(values[m].get(k),0.0) for m in ["PTS","REB","AST"]) for k in ["l5","l10","l20","season","prior"]}
            meta = {
                "anchor": sum(safe_float(metas[m].get("anchor"),0.0) for m in metas),
                "minutes_mult": float(np.mean([metas[m].get("minutes_mult",1.0) for m in metas])),
                "minutes_conf": float(np.mean([metas[m].get("minutes_conf",55.0) for m in metas])),
                "matchup_mult": float(np.mean([metas[m].get("matchup_mult",1.0) for m in metas])),
                "matchup_grade": metas["PTS"].get("matchup_grade","Unknown"),
                "matchup_conf": float(np.mean([metas[m].get("matchup_conf",35.0) for m in metas])),
                "market_weight": float(np.mean([metas[m].get("market_weight",0.0) for m in metas])),
                "sample_quality": float(np.mean([metas[m].get("sample_quality",0.5) for m in metas])),
            }
        else:
            final_proj = comps.get(market, np.nan)
            vals = values.get(market, {})
            meta = metas.get(market, {})

        line = safe_float(row.get("Line"), np.nan)
        edge = final_proj-line if pd.notna(final_proj) and pd.notna(line) else np.nan
        integrity = str(row.get("Data Integrity", "LIMITED")).upper()
        data_conf = 94.0 if integrity == "VERIFIED" else 64.0
        final_conf = float(np.clip(0.36*data_conf + 0.32*meta.get("minutes_conf",55) + 0.18*meta.get("matchup_conf",35) + 0.14*(meta.get("sample_quality",.5)*100), 35, 96))
        sd = _v344_sd(br, market, final_proj, final_conf)
        overp = _v344_probability(line, final_proj, sd)
        underp = 100.0-overp

        # Meaningful direction thresholds. A tiny positive projection difference is
        # NEUTRAL, not an OVER. These are roughly 0.15–0.25 SD by market.
        ...
        baseline_list = _v344_numeric([vals.get("l5"),vals.get("l10"),vals.get("l20"),vals.get("season")]) if isinstance(vals,dict) else []
        recent_votes = 0
        if pd.notna(line) and lean in {"OVER","UNDER"}:
            recent_votes = sum(1 for x in baseline_list if (x>line)==(lean=="OVER"))
        ...
        hit_score = float(np.clip(0.46*sidep + 0.24*final_conf + 0.16*edge_score + 0.14*(40+recent_votes*13), 0, 100))
        ...
        reasons = []
        if lean not in {"OVER","UNDER"}: reasons.append("edge inside neutral/no-play zone")
        if integrity != "VERIFIED": reasons.append("opponent/data context not fully verified")
        if pd.isna(edge) or abs(edge) < official_edge: reasons.append("edge below official threshold")
        if sidep < official_prob: reasons.append("calibrated probability below official threshold")
        if recent_votes < 2: reasons.append("recent/season support is weak")
        if final_conf < 78: reasons.append("projection confidence below 78%")
        official = "PASS" if reasons else ("🔥 OVER" if lean=="OVER" else "⚠️ UNDER")

        row["Projection Before Calibration"] = safe_float(row.get("Projection"), np.nan)
        ...
        row["Opponent Matchup Grade"] = meta.get("matchup_grade","Unknown")
        ...
        row["Minutes Confidence"] = round(meta.get("minutes_conf",55.0),1)
        ...
        row["Projection Audit"] = "L10/L20/season-led Bayesian baseline → bounded minutes once → verified matchup once (max ±1.5%) → small market shrink → uncertainty-calibrated simulation → neutral zone"
        if market == "PRA":
            ...
            row["PRA Identity Check"] = abs(final_proj-(comps["PTS"]+comps["REB"]+comps["AST"])) < 0.005
        rows.append(row)
    ...
    warning = "BALANCED"
    if total >= 8 and over_rate >= .75:
        warning = f"OVER-SKEW WARNING: {overs}/{total} actionable projections lean over. This is a diagnostic—not an automatic side reversal."
    elif total >= 8 and over_rate <= .25:
        warning = f"UNDER-SKEW WARNING: {unders}/{total} actionable projections lean under. This is a diagnostic—not an automatic side reversal."
    summary = {"overs":overs,"unders":unders,"neutrals":neutrals,"total":total,"over_rate":over_rate,"warning":warning}
    ...
    out = out.sort_values(["Official Play Score","Hit Score","Edge"], ascending=[False,False,False])
    save_dataset("projection_board", out)
    return out
...
_render_grouped_player_board_v344_input = render_grouped_player_board

def render_grouped_player_board(mode: str, use_ud_flag: bool, logs_global: pd.DataFrame, master_global: pd.DataFrame):
    _render_grouped_player_board_v344_input(mode, use_ud_flag, logs_global, master_global)
...
def run_full_refresh_with_progress(mode: str, use_ud_flag: bool, logs_global: pd.DataFrame, master_global: pd.DataFrame) -> Tuple[pd.DataFrame, Dict[str, Any]]:
    ...
    try:
        ...
        # Persist the pulled line slate immediately so Moneyline survives a rerun
        # even if the projection build later errors or is still rebuilding.
        try:
            if lines_all is not None and not lines_all.empty:
                _ml_line_snapshot = lines_all.copy()
                _ml_line_snapshot["SavedBoardAt"] = now_iso()
                _ml_line_snapshot["SavedBoardMode"] = mode
                _ml_line_snapshot["LineParserVersion"] = LINE_PARSER_VERSION
                SAVED_LINES_FILE.parent.mkdir(parents=True, exist_ok=True)
                _ml_line_snapshot.to_csv(SAVED_LINES_FILE, index=False)
        except Exception as _ml_line_save_exc:
            status["Moneyline Line Snapshot Warning"] = str(_ml_line_save_exc)[:160]
        ...
        st.session_state[f"wnba_force_live_{mode}"] = False
        st.session_state["wnba_last_refresh"] = now_iso()
        st.session_state["wnba_prefer_projection_cache_after_refresh"] = True
        status["Cards"] = len(board)
        status["Status"] = "complete"
        status["Seconds"] = round(time.time() - started, 2)
        st.session_state["wnba_refresh_today_status"] = status
...
def render_grouped_player_board(mode: str, use_ud_flag: bool, logs_global: pd.DataFrame, master_global: pd.DataFrame):
    ...
    top_cols = st.columns([1.2, 1.0, 1.0, 2.0])
    with top_cols[0]:
        st.info("Use the top REFRESH TODAY button to pull schedule, lines, and projections.")
    with top_cols[1]:
        meta = saved_board_meta()
        st.metric("Last refresh", st.session_state.get("wnba_last_refresh", meta.get("SavedAt", "not yet")))
    with top_cols[2]:
    ...
    with top_cols[3]:
        st.caption(f"Compact rows: Player / Prop / Proj / Edge / Pick / Over %. Line pull locked: {WORKING_PULL_LOCK}. Projection engine: {PROJECTION_ENGINE_VERSION}.")

    prefer_projection_cache = bool(st.session_state.get("wnba_prefer_projection_cache_after_refresh", False))
    saved_df = load_board_snapshot(mode)
    ...
    cached_board = filter_projection_board_for_slate(cached_board, mode)
    if cached_board is not None and not cached_board.empty:
        if prefer_projection_cache:
            st.session_state["wnba_prefer_projection_cache_after_refresh"] = False
    ...
    if mode == "Today":
        render_refresh_today_status()
    st.info(f"No cached {mode.lower()} board is available. Nothing was pulled automatically. Use the refresh button to update schedule, lines, and projections.")
    return pd.DataFrame()
...
UI_MONEYLINE_FIX_VERSION = "V366_COMPACT_BOARD_ML_PERSISTENCE_LOCK"
...
def _lineup_confirmed(row: pd.Series) -> bool:
    txt = str(_strong_text_blob(row, ["Starter Status", "Lineup Status", "Projected Rotation Note", "FallbackLineupRole"]) or "").upper()
    if "PROJECTED STARTER" in txt and "CONFIRMED" not in txt and "OFFICIAL" not in txt:
        return False
    return any(tag in txt for tag in ["CONFIRMED START", "CONFIRMED STARTER", "OFFICIAL STARTER"])
...
def _final_no_bet_flags(row: pd.Series) -> List[str]:
    ...
    # Opening line and CLV are audit-only; missing history does not invalidate a first pregame projection.
...
def _attach_projection_data_readiness(
    ...
    for idx, row in out.iterrows():
        ...
        out.at[idx, "Basketball Recency Days"] = int(global_age)
...
# ============================================================================
# Projection Core V4 patch
# Projection math only. Player Cards, Moneyline rendering, navigation, line
# pulling, saving, and styling remain untouched.
# ============================================================================
PROJECTION_CORE_V4_VERSION = "V4_MINUTES_OPPORTUNITY_20260808"

def _v4_boolish(v):
    if isinstance(v,(bool,np.bool_)): return bool(v)
    if pd.isna(v): return False
    return str(v).strip().upper() in {"1","TRUE","YES","Y","STARTER","STARTING","ACTIVE"}

def _v4_role_class(row):
    txt=str(_strong_text_blob(row,["Starter Status","Lineup Status","Projected Rotation Note","FallbackLineupRole","Role","Injury Status"]) or "").upper()
    if any(x in txt for x in ["OUT","INACTIVE","DOUBTFUL"]): return "OUT"
    if any(x in txt for x in ["MINUTES LIMIT","MINUTE LIMIT","RETURNING"]): return "LIMITED"
    if any(x in txt for x in ["CONFIRMED STARTER","CONFIRMED START","OFFICIAL STARTER"]): return "CONFIRMED_STARTER"
    if "PROJECTED STARTER" in txt: return "PROJECTED_STARTER"
    if any(x in txt for x in ["BENCH","RESERVE"]): return "BENCH"
    return "UNKNOWN"

def _v4_prepare_logs(logs,mode):
    d=standardize_player_logs(logs) if logs is not None and not logs.empty else pd.DataFrame()
    if d is None or d.empty: return pd.DataFrame()
    d=d[(d.get("Played",False).fillna(False).astype(bool)) & (pd.to_numeric(d.get("MIN"),errors="coerce")>0)].copy()
    try: target=slate_target_date(mode or "Today")
    except Exception: target=None
    if target is not None and "GameDate" in d.columns:
        gd=pd.to_datetime(d["GameDate"],errors="coerce")
        d=d[gd.dt.date < target].copy()  # no same-day/postgame leakage
    return d.sort_values(["NameKey","GameDate"],na_position="first")

def _v4_roster_team_map():
    try: r=standardize_rosters(load_dataset("rosters"))
    except Exception: r=pd.DataFrame()
    if r is None or r.empty: return {}
    if "Season" in r.columns and pd.to_numeric(r["Season"],errors="coerce").notna().any():
        s=pd.to_numeric(r["Season"],errors="coerce"); r=r[s==s.max()].copy()
    out={}
    for _,x in r.iterrows():
        k=normalize_name(x.get("Player")); t=_team_key_for_matchup(x.get("Team"))
        if k and t: out[k]=t
    return out

def _v4_line_lookup(lines):
    out={}
    if lines is None or lines.empty: return out
    d=lines.copy()
    for _,r in d.iterrows():
        k=normalize_name(r.get("Player")); m=str(r.get("Market","")).upper(); ln=safe_float(r.get("Line"),np.nan)
        if not k: continue
        out[(k,m,round(ln,3) if pd.notna(ln) else None)]=r.copy()
        out.setdefault((k,m,None),r.copy()); out.setdefault((k,"",None),r.copy())
    return out

def _v4_repair_current_identity(board,lines,logs,mode):
    if board is None or board.empty: return board
    out=board.copy(); roster=_v4_roster_team_map(); ll=_v4_line_lookup(lines); logs_std=_v4_prepare_logs(logs,mode)
    log_team={}
    if logs_std is not None and not logs_std.empty:
        latest=logs_std.sort_values("GameDate").groupby("NameKey",as_index=False).tail(1)
        for _,x in latest.iterrows():
            t=_team_key_for_matchup(x.get("Team")); k=str(x.get("NameKey"))
            if k and t: log_team[k]=t
    for i,r in out.iterrows():
        k=normalize_name(r.get("Matched Player") or r.get("Player")); m=str(r.get("Market","")).upper(); ln=safe_float(r.get("Line"),np.nan)
        lr=ll.get((k,m,round(ln,3) if pd.notna(ln) else None))
        if lr is None:
            lr=ll.get((k,m,None))
        if lr is None:
            lr=ll.get((k,"",None))
        candidates=[pd.Series(lr) if isinstance(lr,(dict,pd.Series)) else None,r]
        away=home=src=""
        for c in candidates:
            if c is None: continue
            try: away,home,src=_event_teams_from_row_anywhere(c)
            except Exception:
                away,home=_parse_event_teams_from_text(c.get("Matchup") or c.get("Event") or c.get("Raw")); src="event"
            if away and home: break
        stored=_team_key_for_matchup(r.get("Team")); line_team=_team_key_for_matchup(lr.get("Team")) if isinstance(lr,(dict,pd.Series)) else ""
        rteam=roster.get(k,""); lteam=log_team.get(k,""); oldopp=_team_key_for_matchup(r.get("Opponent"))
        team=source=""
        if away and home:
            for cand,label in [(line_team,"sportsbook line"),(rteam,"current roster"),(lteam,"latest game log"),(stored,"stored row")]:
                if cand in {away,home}: team,source=cand,label; break
            if not team and oldopp in {away,home}: team=(home if oldopp==away else away); source="event inferred from opponent"
        if not team:
            for cand,label in [(line_team,"sportsbook line"),(rteam,"current roster"),(lteam,"latest game log"),(stored,"stored row")]:
                if cand: team,source=cand,label; break
        opp=""; ha=str(r.get("HomeAway") or ""); matchup=str(r.get("Matchup") or "")
        if away and home and team in {away,home}:
            opp=home if team==away else away; ha="AWAY" if team==away else "HOME"; matchup=f"{away} @ {home}"
        else:
            lineopp=_team_key_for_matchup(lr.get("Opponent")) if isinstance(lr,(dict,pd.Series)) else ""
            for cand in [lineopp,oldopp]:
                if cand and cand!=team: opp=cand; break
        out.at[i,"Team"]=team; out.at[i,"Opponent"]=opp; out.at[i,"Current Team Source"]=source or "unresolved"
        out.at[i,"Current Team Verified"]=bool(team and (not away or team in {away,home}))
        if ha: out.at[i,"HomeAway"]=ha
        if matchup: out.at[i,"Matchup"]=matchup
    return out

def _v4_guard_flat_opponent_context(board):
    if board is None or board.empty: return board
    out=board.copy(); opps=out.get("Opponent",pd.Series("",index=out.index)).map(_team_key_for_matchup)
    v=out.assign(_opp=opps); v=v[v["_opp"].astype(str).ne("")]
    p=pd.to_numeric(v.get("Opponent Pace"),errors="coerce"); d=pd.to_numeric(v.get("Opponent DRtg"),errors="coerce")
    flat=bool(v["_opp"].nunique()>=2 and p.notna().sum()>=2 and d.notna().sum()>=2 and (p.max()-p.min()<=0.05) and (d.max()-d.min()<=0.05))
    for i,r in out.iterrows():
        team=_team_key_for_matchup(r.get("Team")); opp=_team_key_for_matchup(r.get("Opponent")); pace=safe_float(r.get("Opponent Pace"),np.nan); drtg=safe_float(r.get("Opponent DRtg"),np.nan)
        verified=bool((not flat) and team and opp and team!=opp and pd.notna(pace) and pd.notna(drtg))
        out.at[i,"Opponent Context Verified"]=verified
        if flat:
            out.at[i,"Opponent Context Source"]="FLAT_FALLBACK_NEUTRAL"; out.at[i,"Opponent Pace"]=np.nan; out.at[i,"Opponent DRtg"]=np.nan; out.at[i,"Opponent NetRtg"]=np.nan
            out.at[i,"Opponent Market Rank"]=np.nan; out.at[i,"Opponent Matchup Score"]=50.0; out.at[i,"Opponent Matchup Grade"]="Neutral"
            out.at[i,"Opponent Matchup Note"]="Flat league fallback detected; opponent adjustment disabled."
            if str(r.get("Data Integrity","")).upper()=="VERIFIED": out.at[i,"Data Integrity"]="LIMITED"
        else: out.at[i,"Opponent Context Source"]="TEAM_SPECIFIC" if verified else "NEUTRAL_FALLBACK"
    return out

def _v4_minutes_profile(g,row,br):
    old=safe_float(row.get("MIN Proj"),np.nan); role=_v4_role_class(row)
    if role=="OUT": return {"minutes":0.0,"p10":0.0,"p90":0.0,"vol":0.0,"confidence":0.0,"sample":0,"role":role,"note":"player unavailable"}
    mins=pd.to_numeric(g.get("MIN"),errors="coerce").dropna() if g is not None and not g.empty else pd.Series(dtype=float); mins=mins[mins>0]
    if mins.empty:
        fb=old if pd.notna(old) else (_strict_num_from(br,["MIN_l5","MIN_l10","MIN_avg","MIN"],np.nan) if br is not None else np.nan); fb=20.0 if pd.isna(fb) else float(fb)
        return {"minutes":fb,"p10":max(0,fb-4),"p90":min(40.5,fb+4),"vol":3.5,"confidence":45.0,"sample":0,"role":role,"note":"minutes fallback; no pre-slate games"}
    l10=mins.tail(10); l5=mins.tail(5); l3=mins.tail(3); m10=float(l10.median()); m5=float(l5.median()); m3=float(l3.median()); pred=.45*m3+.35*m5+.20*m10
    if g is not None and not g.empty and "Starter" in g.columns and role in {"CONFIRMED_STARTER","BENCH"}:
        sm=g["Starter"].map(_v4_boolish); rg=(g[sm] if role=="CONFIRMED_STARTER" else g[~sm]); rg=rg[pd.to_numeric(rg.get("MIN"),errors="coerce")>0].tail(10)
        if len(rg)>=3: pred=.65*float(pd.to_numeric(rg["MIN"],errors="coerce").median())+.35*pred
    if role=="LIMITED": pred=min(pred,m5)*.90
    p10=float(np.quantile(l10,.10)) if len(l10)>=4 else max(0,m10-3.5); p90=float(np.quantile(l10,.90)) if len(l10)>=4 else min(40.5,m10+3.5); vol=float(l10.std(ddof=0)) if len(l10)>=2 else 2.5
    if role in {"CONFIRMED_STARTER","LIMITED"}: pred=float(np.clip(pred,max(4,p10-2),min(40.5,p90+2.5)))
    else: pred=float(np.clip(pred,max(4,p10),min(40.5,p90)))
    blow=safe_float(row.get("Blowout Risk ML"),np.nan)
    if pd.notna(blow) and blow>=40: p10=max(0,p10-min(3,(blow-40)/12)); p90=min(40.5,p90+1)
    width=max(0,p90-p10); rp=0 if role in {"CONFIRMED_STARTER","BENCH"} else 5 if role=="PROJECTED_STARTER" else 8
    conf=float(np.clip(96-4.8*vol-2.2*abs(m3-m10)-.65*width-rp+min(6,len(l10)*.6),40,95))
    return {"minutes":round(pred,2),"p10":round(p10,2),"p90":round(p90,2),"vol":round(vol,2),"confidence":round(conf,1),"sample":int(len(l10)),"role":role,"note":f"actual L3/L5/L10 medians {m3:.1f}/{m5:.1f}/{m10:.1f}; role {role}"}

def _v4_weighted_rate(g,stat):
    if g is None or g.empty: return np.nan,np.nan,0
    d=g.copy(); d["MIN"]=pd.to_numeric(d.get("MIN"),errors="coerce"); d[stat]=pd.to_numeric(d.get(stat),errors="coerce"); d=d[(d["MIN"]>0)&d[stat].notna()]
    if d.empty: return np.nan,np.nan,0
    def rate(n):
        z=d.tail(n); m=z["MIN"].sum(); return float(z[stat].sum()/m) if m>0 else np.nan
    r5,r10,rs=rate(5),rate(10),rate(len(d)); vals=[(r5,.45),(r10,.35),(rs,.20)]; den=sum(w for v,w in vals if pd.notna(v)); blend=sum(v*w for v,w in vals if pd.notna(v))/den if den else np.nan
    if pd.notna(rs) and rs>0 and pd.notna(blend): blend=float(np.clip(blend,rs*.72,rs*1.28))
    rr=(d.tail(10)[stat]/d.tail(10)["MIN"].replace(0,np.nan)).replace([np.inf,-np.inf],np.nan).dropna(); vol=float(rr.std(ddof=0)) if len(rr)>=2 else np.nan
    return blend,vol,int(len(d.tail(10)))

def _v4_shooting_projection(g,minutes):
    if g is None or g.empty or minutes<=0: return np.nan,"shot data unavailable"
    d=g.copy()
    for c in ["MIN","FGA","FGM","FG3A","FG3M","FTA","FTM"]: d[c]=pd.to_numeric(d.get(c),errors="coerce").fillna(0)
    d=d[d["MIN"]>0]
    if d.empty or d["FGA"].sum()<=0: return np.nan,"shot data unavailable"
    def vr(c,n): z=d.tail(n); return float(z[c].sum()/z["MIN"].sum()) if z["MIN"].sum()>0 else np.nan
    def blend(a,b,c):
        vals=[(a,.45),(b,.35),(c,.20)]; den=sum(w for v,w in vals if pd.notna(v)); x=sum(v*w for v,w in vals if pd.notna(v))/den if den else np.nan
        return float(np.clip(x,c*.72,c*1.28)) if pd.notna(c) and c>0 and pd.notna(x) else x
    fga=blend(vr("FGA",5),vr("FGA",10),vr("FGA",len(d))); tpa=blend(vr("FG3A",5),vr("FG3A",10),vr("FG3A",len(d))); fta=blend(vr("FTA",5),vr("FTA",10),vr("FTA",len(d)))
    d["2PM"]=(d["FGM"]-d["FG3M"]).clip(lower=0); d["2PA"]=(d["FGA"]-d["FG3A"]).clip(lower=0)
    def pct(m,a,n): z=d.tail(n); aa=float(z[a].sum()); return float(z[m].sum()/aa) if aa>0 else np.nan
    def eff(m,a,w=.30):
        r=pct(m,a,10); s=pct(m,a,len(d)); return w*r+(1-w)*s if pd.notna(r) and pd.notna(s) else s if pd.notna(s) else r
    p2=eff("2PM","2PA",.30); p3=eff("FG3M","FG3A",.25); pft=eff("FTM","FTA",.30)
    if any(pd.isna(x) for x in [fga,tpa,fta,p2,p3,pft]): return np.nan,"shot profile partial"
    tpa=float(np.clip(tpa,0,fga)); two=max(0,fga-tpa); ppm=2*two*p2+3*tpa*p3+fta*pft
    return float(max(0,minutes*ppm)),f"FGA/min {fga:.3f}; 3PA/min {tpa:.3f}; FTA/min {fta:.3f}; efficiency shrunk"

def _v4_matchup_factor(row,market):
    if not bool(row.get("Opponent Context Verified",False)): return 1.0
    score=safe_float(row.get("Opponent Matchup Score"),50.0); move=float(np.clip((score-50)/50*.015,-.015,.015)); pct=safe_float(row.get("Opponent Market Allowed Percentile"),np.nan)
    if pd.notna(pct): move+=float(np.clip((pct-.5)*.012,-.006,.006))
    cap=.018 if market=="PTS" else .015; return float(np.clip(1+move,1-cap,1+cap))

def _v4_component(g,mp,row,market):
    mins=safe_float(mp.get("minutes"),np.nan)
    if pd.isna(mins) or mins<=0: return np.nan,0.0,"minutes unavailable"
    rate,vol,n=_v4_weighted_rate(g,market); rate_proj=mins*rate if pd.notna(rate) else np.nan; note=f"{market}/min recent-season shrink"
    if market=="PTS":
        shot,sn=_v4_shooting_projection(g,mins)
        if pd.notna(shot) and pd.notna(rate_proj): proj=.70*shot+.30*rate_proj; note=f"70% opportunity shot model + 30% PTS/min; {sn}"
        else: proj=shot if pd.notna(shot) else rate_proj; note=sn if pd.notna(shot) else note
    else: proj=rate_proj
    if pd.isna(proj): return np.nan,0.0,"component opportunity unavailable"
    mf=_v4_matchup_factor(row,market); proj*=mf; proj=float(np.clip(proj,0,{"PTS":42,"REB":19,"AST":15}.get(market,45)))
    penalty=0 if pd.isna(vol) else min(20,vol*120); conf=float(np.clip(.62*safe_float(mp.get("confidence"),45)+30+min(8,n*.8)-penalty,38,95))
    return proj,conf,f"{note}; matchup factor {mf:.3f}"

def _v4_apply_projection_core(board,logs,base,mode):
    if board is None or board.empty: return board
    out=board.copy(); logs_std=_v4_prepare_logs(logs,mode); groups={str(k):g.copy() for k,g in logs_std.groupby("NameKey")} if logs_std is not None and not logs_std.empty else {}; lookup=_v344_player_lookup(base) if base is not None and not base.empty else pd.DataFrame(); cache={}; rows=[]
    for _,r0 in out.iterrows():
        r=r0.copy(); k=normalize_name(r.get("Matched Player") or r.get("Player")); br=lookup.loc[k] if (not lookup.empty and k in lookup.index) else None; ck=(k,_team_key_for_matchup(r.get("Team")),_team_key_for_matchup(r.get("Opponent")))
        if ck not in cache:
            g=groups.get(k,pd.DataFrame()); mp=_v4_minutes_profile(g,r,br); comps={}; confs={}; notes={}
            for m in ["PTS","REB","AST"]: comps[m],confs[m],notes[m]=_v4_component(g,mp,r,m)
            cache[ck]=(mp,comps,confs,notes,g)
        mp,comps,confs,notes,g=cache[ck]; market=str(r.get("Market","PTS")).upper(); old=safe_float(r.get("Projection"),np.nan)
        if market=="PRA" and all(pd.notna(comps.get(x)) for x in ["PTS","REB","AST"]): proj=float(sum(comps[x] for x in ["PTS","REB","AST"])); conf=float(min(confs.values())); note="PRA exact component sum; confidence = weakest component"
        elif market in comps and pd.notna(comps.get(market)): proj=float(comps[market]); conf=float(confs[market]); note=notes[market]
        else: proj=old; conf=float(min(safe_float(r.get("Final Projection Confidence"),55),60)); note="V4 opportunity fallback to prior mean"
        if g is not None and not g.empty and market in {"PTS","REB","AST","PRA"}:
            vals=pd.to_numeric(g.get(market),errors="coerce")
            if vals.notna().any(): r["L5 Avg"]=round(float(vals.tail(5).mean()),2); r["L10 Avg"]=round(float(vals.tail(10).mean()),2); r["L20 Avg"]=round(float(vals.tail(20).mean()),2); r["Season Avg"]=round(float(vals.mean()),2)
        r["Projection Before V4 Core"]=round(old,2) if pd.notna(old) else np.nan; r["Projection"]=round(proj,2) if pd.notna(proj) else np.nan; r["MIN Proj"]=safe_float(mp.get("minutes"),np.nan); r["Minutes Median"]=safe_float(mp.get("minutes"),np.nan); r["Minutes P10"]=safe_float(mp.get("p10"),np.nan); r["Minutes P90"]=safe_float(mp.get("p90"),np.nan); r["Minutes Low Outcome"]=r["Minutes P10"]; r["Minutes High Outcome"]=r["Minutes P90"]; r["Minutes Volatility"]=safe_float(mp.get("vol"),np.nan); r["Minutes Confidence"]=safe_float(mp.get("confidence"),45); r["Minutes Sample Games"]=int(mp.get("sample",0)); r["Minutes Role Class"]=mp.get("role","UNKNOWN"); r["Minutes Distribution Note"]=mp.get("note",""); r["Chance Under Expected Minutes"]=50.0
        for m in ["PTS","REB","AST"]: r[f"{m} Component Opportunity"]=round(comps.get(m),2) if pd.notna(comps.get(m)) else np.nan; r[f"{m} Component"]=r[f"{m} Component Opportunity"]
        if all(pd.notna(comps.get(x)) for x in ["PTS","REB","AST"]): r["PRA Component PTS"]=round(comps["PTS"],2); r["PRA Component REB"]=round(comps["REB"],2); r["PRA Component AST"]=round(comps["AST"],2); r["PRA Component Sum"]=round(sum(comps.values()),2); r["PRA Identity Check"]=True
        r["Final Projection Confidence"]=round(conf,1); r["Projection Core Version"]=PROJECTION_CORE_V4_VERSION; r["Projection Audit"]=f"V4: current team/opponent -> actual recent minutes -> opportunity {market} -> small verified matchup -> final mean lock"; r["Projection Explanation"]=note; r["WNBA ML Applied To Prop Mean"]=False; r["Prop Mean Game Script Factor"]=1.0; rows.append(r)
    return pd.DataFrame(rows)

def _v4_projection_integrity(board):
    if board is None or board.empty: return board
    out=board.copy()
    for i,r in out.iterrows():
        proj=safe_float(r.get("Projection"),np.nan); mins=safe_float(r.get("MIN Proj"),np.nan); team=_team_key_for_matchup(r.get("Team")); player=normalize_name(r.get("Player")); market=str(r.get("Market","")).upper(); n=safe_float(r.get("Minutes Sample Games"),0)
        ok=(str(r.get("Projection Core Version",""))==PROJECTION_CORE_V4_VERSION and pd.notna(proj) and proj>=0 and pd.notna(mins) and mins>=0 and bool(player) and bool(team) and market in {"PTS","REB","AST","PRA"})
        if ok and n>=5: out.at[i,"Projection Integrity"]="VERIFIED"; out.at[i,"Projection Integrity Note"]="Projection mean verified independently of betting qualification."
        elif ok: out.at[i,"Projection Integrity"]="LIMITED"; out.at[i,"Projection Integrity Note"]="Projection mean available but recent-game sample is limited."
        else: out.at[i,"Projection Integrity"]="BLOCKED"; out.at[i,"Projection Integrity Note"]="Projection core missing required player/team/minutes inputs."
    return out

def make_projection_board(lines, logs, base, mode: Optional[str] = None):
    """Final projection authority; UI, Moneyline, and Underdog pull are unchanged."""
    _mode = mode or "Today"
    board = _make_projection_board_v344_input(lines, logs, base, mode)
    board = _v4_repair_current_identity(board, lines, logs, _mode)
    board = _strict_attach_opponent_context(board, _mode)
    board = _v4_guard_flat_opponent_context(board)
    board = _attach_market_rank_context(board)
    board = _v4_apply_projection_core(board, logs, base, _mode)
    # Projection is locked from here forward. Remaining layers only calculate
    # probabilities, simulations, evidence and betting qualification.
    board = _stable_recalculate_side_fields(board, base)
    board = _stable_attach_context_audit_only(board, base)
    board = _strong_trust_enrich_board(board, _mode)
    board = _stable_recalculate_side_fields(board, base)
    board = _attach_monte_carlo(board, base, sims=WNBA_MONTE_CARLO_SIMS)
    board = _attach_calibration_context(board)
    board = _stable_recalculate_side_fields(board, base)
    board = _apply_winning_play_final_gate(board, base)
    board = _v4_projection_integrity(board)
    if board is not None and not board.empty:
        board = _reattach_line_audit_columns(board, lines)
        board = _attach_player_prop_slate_coverage(board, _mode)
        board = _attach_projection_data_readiness(board, logs, _mode)
        board = _v4_projection_integrity(board)
    board = _ensure_full_live_board_coverage(board, lines, _mode)
    if board is not None and not board.empty:
        board["Projection Engine Version"] = PROJECTION_ENGINE_VERSION
        board["Projection Core Version"] = board.get("Projection Core Version", PROJECTION_CORE_V4_VERSION)
...
with tabs[0]:
    st.markdown("<div class='section-title'>PLAYER CARDS / Compact Board</div>", unsafe_allow_html=True)
    st.caption("Compact Player Cards board: one row per live PTS / REB / AST / PRA market. The Underdog line pull and main-line selector are unchanged.")
...
with tabs[4]:
    ...
    with grade_tabs[1]:
        ...
        if results.empty:
            ...
            c1, c2, c3, c4 = st.columns(4)
            summary = result_record_summary(show_results)
            c1.metric("Final graded rows", summary["total"])
    ...
    with grade_tabs[2]:
        ...
        if not learning.empty:
            ...
            if "Result" in learning.columns:
                learning_summary = result_record_summary(learning)
                if learning_summary["decisions"]:
                    st.metric("Learning final win rate", f"{learning_summary['wins']}-{learning_summary['losses']} ({learning_summary['win_rate']:.1%})")
                else:
...
with tabs[7]:
    ...
    with r4:
        if not learning_df.empty and "Result" in learning_df.columns:
            report_summary = result_record_summary(learning_df)
            st.metric("Tracked win rate", f"{report_summary['wins']}-{report_summary['losses']} ({report_summary['win_rate']:.1%})" if report_summary["decisions"] else "0-0")
        else:
