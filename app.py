...
APP_VERSION = "WNBA v3.6.2 - Fast Rows + Moneyline Runtime Fix"
...
def render_grouped_table_or_cards(proj_df: pd.DataFrame, mode: str, key_prefix: str, default_cards: bool = False) -> pd.DataFrame:
    ...
    for i in range(0, len(player_order), 2):
        cols = st.columns(2)
        for j, player in enumerate(player_order[i:i+2]):
            with cols[j]:
...
with tabs[3]:
    ...
    if ml_board is None or ml_board.empty:
        ...
        render_wnba_ml_system(pd.DataFrame(), "moneyline_tab", mode=ml_mode)
    else:
        render_wnba_ml_system(ml_board, "moneyline_tab", mode=ml_mode)
        ml_cols = [c for c in [
